// ============================================
// NEWSLETTER AUTOMATION - FINAL VERSION
// ============================================
// Uses installable trigger (required for external HTTP requests)
//
// SETUP INSTRUCTIONS:
// 1. Go to Triggers (clock icon ⏰)
// 2. Click "+ Add Trigger"
// 3. Function: onEditHandler
// 4. Event source: From spreadsheet
// 5. Event type: On edit
// 6. Click Save
//
// TROUBLESHOOTING:
// If the trigger stops working (no executions showing up):
// 1. Go to Triggers (⏰)
// 2. Click the three dots (⋮) next to the trigger
// 3. Click "Edit"
// 4. Click "Save" (even if you didn't change anything)
// This refreshes the authorization

function onEditHandler(e) {
  // ---------- CONFIGURATION ----------
  var WEBHOOK_URL = "https://hook.eu2.make.com/a7kogemsw81uwjdor36n8ps6xk14t5xe";
  var TRIGGER_CELL = "B3";
  var TRIGGER_VALUE = "Ready to Generate";

  // ---------- MAIN LOGIC ----------

  // Exit early if no edit event
  if (!e) {
    Logger.log("No edit event detected");
    return;
  }

  var range = e.range;
  var sheet = range.getSheet();
  var sheetName = sheet.getName();
  var cellAddress = range.getA1Notation();
  var cellValue = range.getValue();

  // Log for debugging
  Logger.log("Edit detected: Sheet=" + sheetName + ", Cell=" + cellAddress + ", Value=" + cellValue);

  // Check if this is our trigger cell with trigger value (using trim to handle whitespace)
  if (cellAddress === TRIGGER_CELL && cellValue && cellValue.toString().trim() === TRIGGER_VALUE.trim()) {

    Logger.log("TRIGGER ACTIVATED for sheet: " + sheetName);

    // Update cell to "Processing..." immediately
    sheet.getRange(TRIGGER_CELL).setValue("Processing...");
    SpreadsheetApp.flush(); // Force immediate update

    // Gather data from sheet
    var payload = {
      "sheetName": sheetName,
      "category": sheet.getRange("B2").getValue(),
      "size": sheet.getRange("B5").getValue(),
      "templateId": sheet.getRange("B7").getValue(),
      "nativeAds": sheet.getRange("B4").getValue(),
      "timestamp": new Date().toISOString(),
      "triggerCell": TRIGGER_CELL
    };

    Logger.log("Payload: " + JSON.stringify(payload));

    // Send to Make.com webhook
    var options = {
      "method": "post",
      "contentType": "application/json",
      "payload": JSON.stringify(payload),
      "muteHttpExceptions": true
    };

    try {
      var response = UrlFetchApp.fetch(WEBHOOK_URL, options);
      var responseCode = response.getResponseCode();

      Logger.log("Webhook response code: " + responseCode);

      if (responseCode === 200) {
        Logger.log("SUCCESS: Webhook triggered successfully");
        // Cell will be updated to "Completed" by Make.com workflow
      } else {
        Logger.log("ERROR: Webhook returned code " + responseCode);
        sheet.getRange(TRIGGER_CELL).setValue("Error - Try Again");
      }

    } catch (error) {
      Logger.log("ERROR sending webhook: " + error.toString());
      sheet.getRange(TRIGGER_CELL).setValue("Error - Try Again");
    }

  } else {
    // Not our trigger event, do nothing
    Logger.log("Not a trigger event, ignoring");
  }
}

// ---------- HEALTH CHECK ----------
// Run this periodically to verify the trigger is working
function checkTriggerHealth() {
  var triggers = ScriptApp.getProjectTriggers();
  var found = false;

  Logger.log("=== TRIGGER HEALTH CHECK ===");
  Logger.log("Total triggers found: " + triggers.length);

  for (var i = 0; i < triggers.length; i++) {
    var trigger = triggers[i];
    Logger.log("Trigger " + (i+1) + ":");
    Logger.log("  Function: " + trigger.getHandlerFunction());
    Logger.log("  Event Type: " + trigger.getEventType());

    if (trigger.getHandlerFunction() === "onEditHandler") {
      found = true;
      Logger.log("  ✓ Newsletter trigger is ACTIVE");
    }
  }

  if (!found) {
    Logger.log("⚠️ WARNING: No onEditHandler trigger found!");
    Logger.log("You need to create the trigger manually.");
  } else {
    Logger.log("✓ Trigger health check PASSED");
  }

  return found;
}

// ---------- MANUAL TEST FUNCTION ----------
// Use this to test the webhook without changing B3
function testTrigger() {
  var sheet = SpreadsheetApp.getActiveSheet();
  var sheetName = sheet.getName();
  var TRIGGER_CELL = "B3";

  Logger.log("Testing trigger for sheet: " + sheetName);

  var payload = {
    "sheetName": sheetName,
    "category": sheet.getRange("B2").getValue(),
    "size": sheet.getRange("B5").getValue(),
    "templateId": sheet.getRange("B7").getValue(),
    "nativeAds": sheet.getRange("B4").getValue(),
    "timestamp": new Date().toISOString(),
    "triggerCell": TRIGGER_CELL
  };

  Logger.log("Test payload: " + JSON.stringify(payload));

  var WEBHOOK_URL = "https://hook.eu2.make.com/a7kogemsw81uwjdor36n8ps6xk14t5xe";

  var options = {
    "method": "post",
    "contentType": "application/json",
    "payload": JSON.stringify(payload),
    "muteHttpExceptions": true
  };

  try {
    var response = UrlFetchApp.fetch(WEBHOOK_URL, options);
    Logger.log("Response code: " + response.getResponseCode());
    Logger.log("Response: " + response.getContentText());
  } catch (error) {
    Logger.log("ERROR: " + error.toString());
  }
}
